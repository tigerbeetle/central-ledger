...
import { envOrDefaultNumber, unwrapResponse, unwrapResponseWithError } from "../../testing/util"
...
describe('api/participants/handler', () => {
  before(async () => {
    ...
    const tmpRefactor = envOrDefaultNumber("TMP_REFACTOR", 0) === 0 ? false : true
    harness.configOverride({
      TMP_REFACTOR: tmpRefactor
    })
    logger.warn(`TMP_REFACTOR: ${harness.config.TMP_REFACTOR}`)
