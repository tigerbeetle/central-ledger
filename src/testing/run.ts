...
/**
 * @file run.ts
 * @description Single entrypoint for running unit tests, integration tests etc.
 */
...
async function runCoverage(task: RunTaskCoverage): Promise<void> {
  switch (task.type) {
    ...
    case 'BOTH':
      ...
      spawnSync('npx', ['nyc', 'report', '--reporter=lcov', '--reporter=text-summary'], {
        cwd: PROJECT_ROOT,
        stdio: 'inherit'
      })
      break
  ...
  if (!task.onlyReport) {
    const checkResult = spawnSync('npx', ['nyc', 'check-coverage'], {
      cwd: PROJECT_ROOT,
      stdio: 'inherit'
    })
...
function runCoverageTape(opts: NycOptions): void {
  ...
  const args = [...nycArgs, '--', 'npx', 'tape', ...testFiles]
...
function parseOptions(args: Array<string>, env: NodeJS.ProcessEnv): RunTask {
