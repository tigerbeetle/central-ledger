...
/**
 * Parse a TAP output string into a structured format
 * @param tap - Raw TAP output string
 * @returns Parsed TAP object
 */
...
/**
 * Merge two TAP streams into a single TAP output
 * Tests from the second stream are renumbered to follow the first
 * @param legacy - TAP output from legacy tests
 * @param native - TAP output from native tests
 * @returns Merged TAP output string
 */
...
/**
 * Extract test count from TAP output
 * @param tap - Raw TAP output string
 * @returns Number of tests in the TAP output
 */
export function getTestCount(tap: string): number {
  const parsed = parseTap(tap)
  return parsed.total
}

/**
 * Check if TAP output indicates all tests passed
 * @param tap - Raw TAP output string
 * @returns true if all tests passed
 */
export function allTestsPassed(tap: string): boolean {
  const parsed = parseTap(tap)
  return parsed.ok
}
