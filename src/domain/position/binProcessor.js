...
const processBinsPaymentPrepare = async (bins, trx) => {
  let notifyMessages = []
  let followupMessages = []
  let limitAlarms = []

  const { transferIdList } = await _getTransferIdList(bins)
  ...
  for (const accountID in bins) {
    ...
    actions.forEach(action => {
      assert(action === 'prepare', `processBinsPaymentPrepare() unexpected action: '${action}'.`)
    })
    ...
    assert(accountID !== '0', 'processBinsPaymentPrepare() accountID must not be 0.')
...
const processBinsPaymentFulfil = async (bins, trx) => {
  let notifyMessages = []
  let followupMessages = []
  let limitAlarms = []
...
module.exports = {
  processBins: processBins,
  processBinsPaymentPrepare: processBinsPaymentPrepare,
  processBinsPaymentFulfil: processBinsPaymentFulfil,
  iterateThroughBins
}
