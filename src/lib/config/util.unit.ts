...
describe('lib/config/util', () => {
  ...
  describe('assertKafkaConfig', () => {
    ...
    const weightedMutationOptions = Object.keys(mutationOptions).reduce((acc, curr: string) => {
      ...
      acc = acc.concat(Array(occurrences).fill((curr as unknown as MutationOption), 0, occurrences))
    ...
    const mutateConfigKafka = (input: any, times: number): [any, 'FAIL' | 'PASS'] => {
      ...
      // by default we expect it to be a valid kafka config
      ...
      const _mutateConfig = (input: any): any => {
        ...
        switch (choice) {
          case 'DELETE_ELEMENT': {
            ...
            if (expectPassOrFail === 'FAIL') {
              break;
            }

            // Non fatal - final leaf nodes on the consumer or producer options
            if (path.match(/config\|(options|rdkafkaConf|topicConf)\|(.*)/)) {
              break;
            }
          ...
          case 'MUTATE_ACTION_TOPIC_MAP': {
            ...
            if (newValue === null || newValue === 'test-string') {
              break;
            }
        ...
        switch (expectPassOrFail) {
          ...
          case 'FAIL': {
            ...
            try {
              ...
              console.log(`expected mutated kafka config to fail validation: (iteration: ${idx})`)
              console.log(JSON.stringify(mutated, null, 2))
              testFail = true
            } catch (err) {
              testFail = false
            }
            assert(testFail === false, 'Expected assertKafkaConfig() to throw on invalid input.')
            break;
          }
          ...
          case 'PASS': {
            try {
              ...
              console.log(`expected mutated kafka config to pass validation: (iteration: ${idx})`)
              console.log(JSON.stringify(mutated, null, 2))
